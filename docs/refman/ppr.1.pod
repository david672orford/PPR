...
This is a temporary experimental option which will be removed in PPR 1.50 when 
the commentary and responder systems are integrated.

This option indicates that, while this job is being printed, commentary
messages of the categories respresented by the I<integer> should be
...
of commentary desired.  See the section on commentators in "PPR, a Print Spooler
For PostScript" for a list of the values.

=item B<--commentator> I<method>

This is a temporary experimental option which will be removed in PPR 1.50 when 
the commentary and responder systems are integrated.

This option indicates that the method I<method> should be used to send
commentary messages to the user.  If this option is not specified, then the
value is read from the environment variable PPR_COMMENTATOR.  If that variable
is not defined, then the responder method is used.

=item B<--commentator-address> I<address>

This is a temporary experimental option which will be removed in PPR 1.50 when 
the commentary and responder systems are integrated.

This option indicates the address to which commentary messages should be sent.
This address should be in a format suitable for the commentator method being used.
If this option is not specified, then the value is read from the environment
variable PPR_COMMENTATOR_ADDRESS.  If that variable is not defined, then the
responder address is used.

=item B<--commentator-options> I<list>

This is a temporary experimental option which will be removed in PPR 1.50 when 
the commentary and responder systems are integrated.

This option is used to specify a space separated list of name=value pairs.  This
list is passed the the commentator program.  The meaning of the name=value pairs
depends entirely uppon the commentator program.

If this option is not specified, then the value is read from the environment
variable PPR_COMMENTATOR_OPTIONS.  If that value is not defined, then
an empty option list is used.  (The responder options are I<not> used!)
...
This switch inserts PostScript setup code for printer features.  Since
...
Read any copy count which appears in a document "%%Requirements:" comment
and print that many copies.  The default is B<-R ignore-copies>.
...
should be printed.  It does not work.
