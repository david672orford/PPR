...
Prior to version 2.00, PPR had a complicated system for stripping resources
(such as procedure sets and fonts) out of jobs and reinserting them when the
job was sent to the printer. While useful in theory, only LaserWriter 8 (a
Macintosh printer driver) to advantage of it.  Other than that, it simply caused problems because
the writers of printer drivers were not careful to change the resource version
numbers when the changed the resources. As a result one print job could 'poison' 
the cache with versions of resources which were incompatible
