...
The option "snmp_community=" tell the interface what community name (password)
to use when using SNMP to fetch the printer status.  The default value is
"public".  Almost all printers allow read-only access with the community
name "public".
