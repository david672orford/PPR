...
A good way to stop B<papsrv> is with the shell script
B<papsrv_kill>.  This script reads the process id of
B<papsrv> from the file F</var/spool/ppr/run/papsrv.pid> and kills B<papsrv> by
sending it SIGTERM.  It then waits for B<papsrv> to shut down, then exits
itself.  When B<papsrv_kill> is done it is safe to start a new B<papsrv>.
...
Use the name that appears in the "%%For:" line in the PostScript.
...
to print without having mounted a file sharing volume, papsrv detects this
and tell the Macintosh that a PostScript error has occured.  In order for
...
choose print in any application, then press the "Options" button and then
...
with the HIDE_AUFS_SECURITY option.  (With
HIDE_AUFS_SECURITY enabled, only root can tell if a user has a volume
mounted.  It is unclear to me how the option increases security anyway,
except perhaps if the security directory is F</tmp>.)
...
L<ppop>(1), L<ppr>(1), L<ppad>(8), L<pprd>(8)
"PPR, a PostScript Print Spooler", "Installing and Using PPR".

=head1 HISTORY

PPR was written at Trinity College during 1993--1998.
