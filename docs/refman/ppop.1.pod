...
The B<--verbose> switch will cause some commands to display more detailed 
information.  For example, the B<ppop status> command will display status
information from LaserWriter messages, PJL messages, and SNMP separately.
...
This is the name of the commentator program which will be used.
See B<ppr --commentator>.

=item commentator-address

This is the address which will be passed to the commentator program.
See B<ppr --commentator-address>.

=item commentator-options

This is the space-separated list of options which will be passed to the
commentator program.  See B<ppr --commentator-options>.
...
group, all printers in the indicated group, or, if "B<all>" is used, all
