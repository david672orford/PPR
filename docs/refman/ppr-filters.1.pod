...
The default value for the "charset=" option is "ISOLatin1".  The default for
toe "fontfamily=" option is "fixed".
...
Normally, the file name or the string specified with
the B<-C> switch (if present) will be used as the title.  You may override
the title with the B<-o title=> option.

The B<-o> option "B<width=>" specifies
the value for B<pr>'s B<-w> switch and "B<length=>" specifies a value for
B<pr>'s B<-l> option.  Since, after being processed by B<pr>, the file is
passed through the "lp" filter, lines will be wrapped at the length
specified by the B<-o width=> switch.
...
sets the title to print a the top of each page.  If this switch
is absent, a default title (probably derived from the B<-C> switch
or the input file name) will be used.
...
These options inform the filter of the size of the fonts selected.  These
will normally only have to be used if the "fontnormal=" or "fontbold="
options are used.  The dimensions are expressed as a decimal fraction of the
point size.  For example, a typical setting for Courier is
C<-o 'charwidth=0.6 charheight=1.0'>.
