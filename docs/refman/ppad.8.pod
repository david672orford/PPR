...
appropriate section in the L<ppad_interfaces(8)> manpage to learn what
...
=item B<ppad rip> I<printer> [I<rip> I<driver> I<driver_output_language> [I<options>]]

This option is new and experimental in PPR version 1.44.

PPR has the ability to execute external Raster Image Processors such as Ghostscript to
convert PostScript to a form usable by non-PostScript printers.  It does so when it
finds a "*pprRIP:" line in the PPD file or if this command has been used to set a RIP
in the printer's configuration file.  RIP settings set with this command override those
in the PPD file.

The parameter I<rip> indicates the Raster Image Processor to use.  Currently only B<gs>
which selects GhostScript is supported.  PPR will find the actual Ghostscript executable by
fetching the first value of B<gs=> in the B<[ghostscript]> section of F</etc/ppr/ppr.conf>.

The parameter I<driver> selects the Ghostscript driver.  Run B<gs -h> to get a list
of the drivers compiled into your copy of Ghostscript.  For example, you might choose
"cdj550" to use the HP DeskJet 550C driver.

If your copy of Ghostscript includes the driver called "uniprint", then you may instead
specify the name of a F<.upp> file.  For example, you might choose "cdj550.upp" to select
the Uniprint driver for the same printer.  If the F<.upp> file is in Ghostscript's search
path, there is no need to specify the directory.

The parameter I<driver_output_language> is the name of the printer language which the
selected Ghostscript driver will produce output in.  It is used as the parameter to the
PJL B<@PJL ENTER LANGUAGE> command.  If you will not be using the B<pjl> or B<signal/pjl>
jobbreak method, then the actual value doesn't matter, you can set it to "?".

The option I<options> contains additional options for Ghostscript.  The syntax has not
yet been defined and any value you enter will be ignored.  This option is optional.

To revert to the RIP settings in the PPD file, execute this command with just the
printer name.
...
such as when the job is being submitted through Samba, B<lprsrv>, or B<papsrv>.
...
Another option one should not include in a switchset is B<-m> (which sets the
responder method).  Why?  Because the method by which PPR should send messages
to the person who submitted the job depends very much on how the job arrived.
For example, if the job was submitted by a Unix shell user on the server, then
the B<write> command can be used to send a message to his terminal.  But if
the user is Microsoft Windows user connecting through Samba, then the
B<smbclient> program should be used to send messages.  For this reason, network
servers such as Samba, B<lprsrv>, and B<papsrv> set the B<-m> and B<-r> options
...
should be selected with the "ppad ppd" command before this command is
used.  The information which the user supplies is used to answer queries
from Macintosh clients.  It is likely that this information will be used for
other purposes in the future.  L<papsrv> will not pick up new information until
it is restarted.  Running "ppad ppd" will undo "ppad ppdopts", leaving all
of the feature information unspecified.
...
PPR ACLs are stored in files in B</etc/ppr/acl>.  The ACL system is still under
development, so no furthur information is available at this time.
...
ppop(1), ppr(1), papsrv.conf(5), mfmodes.conf(5), papsrv(8), papsrv(8),
lprsrv(8), ppuser(8) "PPR, a PostScript Print Spooler", "Installing and
Using PPR".
