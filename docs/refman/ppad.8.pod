...
appropriate section in the L<ppad-interfaces(8)> manpage to learn what
...
This option is new and experimental in PPR version 1.44.  It has already changed once 
during the beta testing period and may change again.
...
The parameter I<rip> indicates the Raster Image Processor to use.  If the value is B<gs>,
then PPR will use the copy of Ghostscript pointed to by the first value of B<gs=> in the 
B<[ghostscript]> section of F</etc/ppr/ppr.conf>.  If it is B<ppr-gs> then it will use
the special copy of Ghostscript from the PPR-Ghostscript distribution.  (You must of course 
download this special Ghostscript distribution from the place you obtained PPR and install
it before this option will be useful.)

The parameter I<output_language> is the name of the printer language which the
...
jobbreak method, then the actual value doesn't matter, you can set it to "x".

The option I<options> contains options for Ghostscript.  These should select the desired 
Ghostscript device driver.
...
other purposes in the future.  L<papsrv(8)> will not pick up new information until
...
ppop(1), ppr(1), papsrv.conf(5), mfmodes.conf(5), papsrv(8),
