...
Tcl_LinkVar(Tcl_Interp *interp, char *varName, char *addr, int type)
                       		/* Interpreter in which varName exists. */
                  		/* Name of a global variable in interp. */
               			/* Address of a C variable to be linked
				 * to varName. */
             			/* Type of C variable: TCL_LINK_INT, etc. 
...
Tcl_UnlinkVar(Tcl_Interp *interp, char *varName)
                       		/* Interpreter containing variable to unlink. */
                  		/* Global variable in interp to unlink. */
{
    Link *linkPtr;
...
static char *
LinkTraceProc(ClientData clientData, Tcl_Interp *interp, char *name1, char *name2, int flags)
                          	/* Contains information about the link. */
                       		/* Interpreter containing Tcl variable. */
...
static char *
StringValue(Link *linkPtr, char *buffer)
                  		/* Structure describing linked variable. */
                 		/* Small buffer to use for converting
