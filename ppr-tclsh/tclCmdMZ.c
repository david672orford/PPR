    ...
    char **argv;			/* Argument strings. */
{
	...
	if (start == NULL) {
	    ...
	    if (indices) {
		snprintf(info, sizeof(info), "%d %d", (int)(start - string), (int)(end - string - 1));
...
#   define STATIC_SIZE 5
	...
	*dst = *fmt;
	...
	switch (*fmt) {
	    ...
	    default:
		snprintf(interp->result, TCL_RESULT_SIZE+1, "bad scan conversion character \"%c\"", *fmt);
		code = TCL_ERROR;
		goto done;
	}
    ...
    char **argv;			/* Argument strings. */
{
	...
	snprintf(interp->result, TCL_RESULT_SIZE+1, "%d", (int)strlen(argv[2]));
	return TCL_OK;
