#! /bin/sh
#
# mouse.trincoll.edu:~ppr/src/misc_filters/xpm.sh
...
if [ -n "$COLOUR" ]
	then
	$XPMTOPPM | $PNMDEPTH 255 | $PNMTOPS $RESOLUTION | grep -v '^%%Title:'
	else
	$XPMTOPPM | $PPMTOPGM | $PNMDEPTH 255 | $PNMTOPS $RESOLUTION | grep -v '^%%Title:'
	fi
